...
export function CashMovementsPage() {
  ...
  return <div className="page-stack"><PageHeader eyebrow="Caja" title="Movimientos de efectivo" description="Ledger inmutable con filtros y paginación del servidor." />
    ...
    <section className="panel"><ErpTable columns={movementColumns} rows={list.data?.data} rowKey={(row) => row.id} loading={list.isLoading} error={list.error ? apiErrorMessage(list.error) : undefined} onRetry={() => void list.refetch()} emptyTitle="No se encontraron movimientos" /><Pagination meta={list.data?.meta} onPageChange={(page) => filters.update({ page }, false)} /></section>
  </div>;
}
