...
import { PartialCutPanel } from "./PartialCutPanel";
...
export function CashRegisterDetailPage() {
  ...
  const currentSummary = useQuery({ queryKey: queryKeys.cashSessionSummary(current.data?.id ?? "none"), queryFn: () => cashApi.summary(current.data!.id), enabled: Boolean(current.data?.id) && hasPermission("cash-movements.read") });
  const recentClosed = useQuery({ queryKey: queryKeys.cashSessions({ cashRegisterId: id, status: "CLOSED", page: 1, limit: 5 }), queryFn: () => cashApi.sessions({ cashRegisterId: id, status: "CLOSED", page: 1, limit: 5 }), enabled: hasPermission("cash-sessions.read") });
  ...
  return <div className="page-stack"><PageHeader eyebrow="Caja" title={`${row.code} · ${row.name}`} description={row.description || "Caja física"} actions={<><Link className="button button--secondary" to={`/app/cash/sessions?cashRegisterId=${id}`}>Ver sesiones</Link>{hasPermission("cash-registers.manage") ? <><Link className="button button--secondary" to={`/app/cash/registers/${id}/edit`}>Editar</Link><Button variant={row.active ? "danger" : "primary"} onClick={() => setLifecycleConfirm(true)}>{row.active ? "Desactivar" : "Activar"}</Button></> : null}</>} />
    <section className="panel detail-grid"><div className="detail-card"><h2>Identidad</h2><dl><div><dt>Código</dt><dd>{row.code}</dd></div><div><dt>Nombre</dt><dd>{row.name}</dd></div><div><dt>Estado</dt><dd><StatusBadge active={row.active} /></dd></div></dl></div>
      <div className="detail-card"><h2>Sesión actual</h2>{!hasPermission("cash-sessions.read") ? <p className="muted">Sin permiso para consultar sesiones.</p> : current.isLoading ? <p>Cargando…</p> : current.data ? <dl><div><dt>Estado</dt><dd><SessionBadge status={current.data.status} /></dd></div><div><dt>Abierta</dt><dd>{formatDateTime(current.data.openedAt)}</dd></div><div><dt>Efectivo inicial</dt><dd>{formatMoney(current.data.openingAmount)}</dd></div>{currentSummary.data ? <><div><dt>Cobros de venta hoy</dt><dd>{formatMoney(currentSummary.data.movementTotals.SALE_PAYMENT)}</dd></div><div><dt>Efectivo esperado ahora</dt><dd><strong>{formatMoney(currentSummary.data.expectedCash)}</strong></dd></div></> : null}<div><dt>Acción</dt><dd><Link className="table-link" to={`/app/cash/sessions/${current.data.id}`}>Ver sesión y corte parcial</Link></dd></div></dl> : <p className="muted">No hay sesión abierta.</p>}</div></section>
    {recentClosed.data && recentClosed.data.data.length > 0 ? <section className="panel"><div className="section-heading"><div><h2>Últimos cierres</h2><p>Comparativa rápida de los cierres recientes de esta caja.</p></div><Link className="button button--ghost" to={`/app/cash/sessions?cashRegisterId=${id}`}>Ver todas</Link></div><div className="table-wrap"><table><thead><tr><th>Cierre</th><th>Inicial</th><th>Contado</th><th>Diferencia</th></tr></thead><tbody>{recentClosed.data.data.map((session) => <tr key={session.id}><td><Link className="table-link" to={`/app/cash/sessions/${session.id}`}>{session.closedAt ? formatDateTime(session.closedAt) : "—"}</Link></td><td>{formatMoney(session.openingAmount)}</td><td>{formatMoney(session.countedAmount ?? "0")}</td><td>{session.differenceAmount != null ? formatMoney(session.differenceAmount) : "—"}</td></tr>)}</tbody></table></div></section> : null}
    {row.active && hasPermission("cash-sessions.open") && !current.data ? <form className="panel erp-form" onSubmit={(event) => { event.preventDefault(); setOpenConfirm(true); }}><div className="section-heading"><div><h2>Abrir sesión</h2><p>El sistema garantiza una sola sesión abierta por caja.</p></div></div><FormFeedback error={open.error ? apiErrorMessage(open.error) : null} /><div className="form-grid"><Field label="Efectivo inicial" htmlFor="opening-amount" required><input id="opening-amount" required inputMode="decimal" placeholder="0.00" value={opening.amount} onChange={(event) => setOpening({ ...opening, amount: event.target.value })} /></Field><Field label="Notas" htmlFor="opening-notes"><textarea id="opening-notes" maxLength={500} value={opening.notes} onChange={(event) => setOpening({ ...opening, notes: event.target.value })} /></Field></div><div className="form-actions"><Button type="submit" disabled={!isMoneyAtLeast(opening.amount, "0")}>Revisar apertura</Button></div></form> : null}
    ...
    <ConfirmDialog open={openConfirm} title="Abrir sesión de caja" description={`Se abrirá con ${opening.amount ? formatMoney(opening.amount) : "el monto indicado"}. La validación final corresponde al sistema.`} confirmLabel="Abrir sesión" loading={open.isPending} onCancel={() => setOpenConfirm(false)} onConfirm={() => open.mutate()} />
  </div>;
}
