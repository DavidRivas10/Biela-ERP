...
function RoleEditor({ id, initial }: { id?: string; initial?: Role }) {
  ...
  const mutation = useMutation({ mutationFn: (body: RoleInput) => id ? rolesApi.update(id, body) : rolesApi.create(body), onSuccess: async (row) => { client.setQueryData(queryKeys.role(row.id), row); await client.invalidateQueries({ queryKey: queryKeys.rolesRoot }); void navigate(`/app/admin/roles/${row.id}`, { replace: true }); } });
